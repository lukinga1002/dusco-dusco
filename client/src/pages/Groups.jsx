      ...
      <div className="px-5 space-y-3 pb-6">
        {groups.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-sm text-gray-500 mt-1 mb-6">Create a savings group or get invited to one</p>
          ...
          <Link key={g.id} to={`/groups/${g.id}`}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-dark text-sm truncate">{g.name}</h3>
                <p className="text-[10px] text-gray-500 capitalize">{g.memberCount} members · {g.contributionFrequency}</p>
                <p className="text-[10px] text-gray-400 font-mono">{g.duscoNumber}</p>
              </div>
