...
export default function PrivacyBalance({ amount, className, prefix = "TZS", size = "lg" }) {
  ...
  return (
    <div className="flex min-w-0 max-w-full flex-wrap items-center gap-2.5">
      <span
        className={cn(
          "min-w-0 break-words font-display font-semibold tracking-tight tabular-nums text-dusco-ink transition-all duration-300",
          sizeClass,
      ...
      <button
        type="button"
        onClick={() => setRevealed((r) => !r)}
        className="shrink-0 p-1.5 -ml-1 rounded-lg text-dusco-ink-mute hover:text-dusco-ink hover:bg-dusco-sand transition-colors"
